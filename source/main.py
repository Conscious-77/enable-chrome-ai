...
def main(close_chrome_mode='ask'):
    ...
    if close_chrome_mode == 'yes':
        should_shutdown_chrome = True
    elif close_chrome_mode == 'no':
        should_shutdown_chrome = False
    else:
        should_shutdown_chrome = ask_yes_no(
            'Close running Chrome before patching? (recommended)',
            default=True,
        )
    terminated_chromes = set()
    if should_shutdown_chrome:
        terminated_chromes = shutdown_chrome()
        if len(terminated_chromes) > 0:
            print('Shutdown Chrome')
    else:
        print('Skipping Chrome shutdown. If Local State is locked, patching may fail.')
    ...
    if should_shutdown_chrome and len(terminated_chromes) > 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=('ask', 'yes', 'no'),
        default='ask',
        help='whether to close Chrome before patching',
    )
    args = parser.parse_args()
    main(close_chrome_mode=args.close_chrome)
